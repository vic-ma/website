...
The word suggestions are generated by a *word suggestion algorithm*.



A basic word suggestion algorithm would output words that have the same length as the current slot, and that match any letters already in the slot. For example, if the slot is `C A _ S`, then the basic word suggestion algorithm would only suggest four-letter words that start with *CA* and end in *S*---such as *CATS* or *CABS*, but not *COTS*.
