...
The word suggestions are generated by a *word suggestion algorithm*. At a bare minimum, this algorithm should only output words that have the same length as the current slot, and that match any letters already in the slot. For example, if the slot is `C A _ S`, then the word suggestion algorithm should only suggest four-letter words that start with *CA* and end in *S*---such as *CATS* or *CABS*, but not *COTS*.

### The problem

There is a problem with this basic word suggestion algorithm, however. Consider this grid:
```
+---+---+---+---+
|   |   |   | Z |
+---+---+---+---+
